 ...
 */
export const Dropdown = ({ trigger, children, align = 'start', className = '', closeOnItemClick = true }) => {
  ...
  const handleKeyDown = useCallback(
    (event) => {
      ...
      const menuItems = menuRef.current?.querySelectorAll('[role="menuitem"], .dropdown-item');
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setTimeout(() => {
        const firstItem = menuRef.current?.querySelector('[role="menuitem"], .dropdown-item');
        firstItem?.focus();
      }, 0);
    } else {
 ...
 */
export const DropdownItem = ({ children, onClick, active = false, href, className = '', ...props }) => {
  ...
  return (
    <button type="button" className={itemClass} role="menuitem" onClick={handleClick} {...props}>
      {children}
    </button>
  );
};
